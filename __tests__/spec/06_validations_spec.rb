...
RSpec.describe 'Validations API', :order => :defined do
  ...
  it 'validate stacked discounts (skipped)', :order => :twelweth do
    base_voucher_code = @voucherify_data.get_voucher().code
    random_voucher_codes = generate_random_voucher_codes(base_voucher_code, 29)
    voucher_codes = [base_voucher_code] + random_voucher_codes
